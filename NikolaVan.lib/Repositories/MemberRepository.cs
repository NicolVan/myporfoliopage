...
namespace NikolaVan.lib.Repositories
{
    public class MemberRepository : _BaseRepository
    {
        public const string NikolaVanMemberTypeAlias = "Member";
        public const string NikolaVanMemberAdminRole = "Admin";
        public const string NikolaVanMemberCustomerRole = "Klient";
        ...
        IEnumerable<IMember> GetAllMembers(string sortBy = "Name", string sortDir = "ASC")
        {
            ...
            return this.MemberService.GetAll(0, _PagingModel.AllItemsPerPage, out totalRecords, sortBy, sortDir == "DESC" ? Direction.Descending : Direction.Ascending, MemberRepository.NikolaVanMemberTypeAlias);
        }
        ...
        Member CreateCopyFrom(IMember imember)
        {
            ...
            member.IsAdminUser = System.Web.Security.Roles.IsUserInRole(member.Username, MemberRepository.NikolaVanMemberAdminRole);
            member.IsCustomerUser = System.Web.Security.Roles.IsUserInRole(member.Username, MemberRepository.NikolaVanMemberCustomerRole);
        ...
        public MembershipCreateStatus Insert(PluginController ctrl, Member member)
        {
            ...
            var registerModel = ctrl.Members.CreateRegistrationModel(MemberRepository.NikolaVanMemberTypeAlias);
            ...
            if (status == MembershipCreateStatus.Success)
            {
                ...
                {
                    System.Web.Security.Roles.AddUserToRole(member.Username, MemberRepository.NikolaVanMemberAdminRole);
                }
                if (member.IsCustomerUser)
                {
                    System.Web.Security.Roles.AddUserToRole(member.Username, MemberRepository.NikolaVanMemberCustomerRole);
                }
            }
        ...
        public MembershipCreateStatus Update(Member member, bool updatePermissions)
        {
            ...
            if (updatePermissions)
            {
                ...
                {
                    System.Web.Security.Roles.RemoveUserFromRole(member.Username, MemberRepository.NikolaVanMemberAdminRole);
                }
                if (member.IsCustomerUser)
                {
                    System.Web.Security.Roles.AddUserToRole(member.Username, MemberRepository.NikolaVanMemberCustomerRole);
                ...
                {
                    System.Web.Security.Roles.RemoveUserFromRole(member.Username, MemberRepository.NikolaVanMemberCustomerRole);
                }
            }
    ...
    public class MemberRolesInfo
    {
        ...
        {
            return IsMemberInRole(service, member, MemberRepository.NikolaVanMemberAdminRole, ref htAdmin);
        }

        public bool IsCustomer(IMemberService service, IMember member)
        {
            return IsMemberInRole(service, member, MemberRepository.NikolaVanMemberCustomerRole, ref htCustomer);
        }
